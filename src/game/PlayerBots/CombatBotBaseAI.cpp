...
uint32 SelectHunterBotPetEntry()
{
    static std::vector<uint32> const petEntries = []()
    {
        ...
        for (const auto& itr : sObjectMgr.GetCreatureInfoMap())
        {
            ...
            // Exclude rare/elite/world-boss tier beasts — hunters cannot tame
            // elites in vanilla, and rare/boss models look out of place on a
            // bot pet that is rescaled to its owner's level.
            if (cInfo->rank != CREATURE_ELITE_NORMAL)
                continue;

            // Cap at max player level. Excludes high-level zone beasts
            // (e.g. Silithus / EPL) that would only confuse low-level bots.
            if (cInfo->level_max > PLAYER_MAX_LEVEL)
                continue;
...
SpellCastResult CombatBotBaseAI::DoCastSpell(Unit* pTarget, SpellEntry const* pSpellEntry)
{
    ...
    if (result == SPELL_CAST_OK && me->GetBattleGroundTypeId() == BATTLEGROUND_BR)
    {
        uint32 const tgtClass = pTarget->IsPlayer()
            ? static_cast<Player const*>(pTarget)->GetClass() : 0u;
        sLog.Out(LOG_BG, LOG_LVL_BASIC,
                 "[BRSkill] who=bot name=%s class=%u instance=%u cast %s on %s(class %u)"
