...
namespace
{
    ...
    // Deliberately a separate connection from CharacterDatabase's own query connections (see
    ...
    // Same reasoning as DbWriteOutbox::LogHeartbeatIfDue() / realmd's DbHealthMonitor - a probe
    // that's silently wedged looks identical, from the log alone, to one that's healthy and just
    // has nothing to report; a periodic line even when nothing changed makes that distinguishable.
    ...
    void ProbeThreadMain()
    {
        ...
        while (!g_stop)
        {
            // mysql_ping() is a real network round trip - only that reliably tells "connected"
            // apart from "merely not yet noticed as dead" (see HPHA.md Phase1's "会不会误判" entry).
            ...
            if (!ok)
            {
                ...
                g_probeConn = std::make_unique<MySQLConnection>(CharacterDatabase);
            ...
            if (wasHealthy && !ok)
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "[DbHealthMonitor] CharacterDatabase probe failed - mail/auction house/trade/petition actions will be rejected until it recovers.");
            else if (!wasHealthy && ok)
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "[DbHealthMonitor] CharacterDatabase probe recovered - accepting mail/auction house/trade/petition actions again.");

            time_t const now = time(nullptr);
            if (now - g_lastHeartbeatLogTime >= time_t(HEARTBEAT_LOG_INTERVAL_SEC))
            {
                g_lastHeartbeatLogTime = now;
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "[DbHealthMonitor] heartbeat - CharacterDatabase healthy=%s", ok ? "yes" : "no");
            }
