...
void WorldSession::HandleGroupInviteOpcode(WorldPackets::Group::GroupInvite const& packet)
{
    ...
    if ((leader->IsTianxuan() && !leader->GetQuestRewardStatus(920501)) || (player->IsTianxuan() && !player->GetQuestRewardStatus(920501)))
    {
        ...
        if (diff > 5)
        {
            ChatHandler(leader).SendSysMessage("[天选者] 天选者组队等级差不能超过 5 级。");
            ChatHandler(player).SendSysMessage("[天选者] 天选者组队等级差不能超过 5 级。");
            SendPartyResult(PARTY_OP_INVITE, packet.memberName, ERR_IGNORING_YOU_S);
            return;
        }
    }
...
void WorldSession::HandleLootRoll(WorldPackets::Loot::LootRoll const& packet)
{
    ...
    if (GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_NO_PLAY_TIME) && !GetPlayer()->GetMap()->IsRaid())
    {
