...
void WorldSession::HandleBuyItemInSlotOpcode(WorldPackets::Item::BuyItemInSlot const& packet)
{
    ...
    if (pCreature)
    {
        if (pCreature->IsGuildBank())
        {
            GetPlayer()->BuyItemFromGuildVendor(packet.vendorGuid, packet.item, packet.count, bag, packet.bagslot);
        }
        else
        {
...
void WorldSession::HandleBuyItemOpcode(WorldPackets::Item::BuyItem const& packet)
{
    ...
    if (pCreature)
    {
        if (pCreature->IsGuildBank())
        {
            GetPlayer()->BuyItemFromGuildVendor(packet.vendorGuid, packet.item, packet.count, NULL_BAG, NULL_SLOT);
            return;
        }
    }
