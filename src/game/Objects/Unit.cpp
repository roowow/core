...
void Unit::Kill(Unit* pVictim, SpellEntry const* spellProto, bool durabilityLoss)
{
    ...
    if(pPlayerVictim && pPlayerTap)
    {
        if( pPlayerTap != pPlayerVictim)
        {
            ...
            // kill announce (BG-wide) + AFK contribution
            sOOMgr.HandleBGKillAnnounce(pPlayerTap, pPlayerVictim);
            if (!pPlayerTap->IsBot() && !pPlayerTap->IsGameMaster() && pPlayerTap->InBattleGround())
                if (BattleGround* afkBg = pPlayerTap->GetBattleGround())
...
bool Unit::AddSpellAuraHolder(SpellAuraHolder* holder)
{
    ...
    // must be checked too, otherwise players who ever finished 32003 would be permanently blocked.
...
{
    // Merged 2026-08-25 (upstream/development): upstream rewrote this to use the structured
    // WorldPackets::Combat::MeleeAttackingStateUpdate builder instead of manual WorldPacket
    // byte-packing (kept below); our side had already routed the send through
    // SendCombatLogMessageToSet() for 序3+4 (see HPHA.md "网络带宽优化") - was:
    // SendMessageToSet(std::move(packet), true). SendCombatLogMessageToSet() needs a raw
    // WorldPacket, same conversion already used in SpellCaster::SendHealSpellLog()/
    // SendSpellNonMeleeDamageLog() for the same reason.
    ...
    SendCombatLogMessageToSet(binaryPacket, damageInfo->target);
}
