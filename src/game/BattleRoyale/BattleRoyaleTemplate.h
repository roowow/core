...
inline BattleRoyaleTemplate& GetGMIslandTemplate()
{
    static BattleRoyaleTemplate tmpl = []() -> BattleRoyaleTemplate
    {
        ...
        // 绕圈高度130.120071是用户实地飞行找的合适高度（GPS实测值）。半径按用户要求
        // 放大2倍到120码（参照海加尔山60->120的做法）。轨道节点0 = 圆心正东120码。
        //
        // ⚠️ 已知冲突（用户已确认"先120我测试下"，暂不处理，留到实测后再看）：
        // GM岛西侧硬边界（openWorldMaxY=16348.754883）离圆心只有51.46码，120码半径的
        // 绕圈轨道在朝西的弧线上会飞出边界约68.5码——跟海加尔山不同，海加尔山当初特意
        // 核对过"120码半径的圆完全落在边界内"才定下来，GM岛这次没有这个空间。真人/
        // 机器人绕圈飞过西侧弧线时可能触发OPEN_WORLD硬边界的"越界淘汰传送回原位"逻辑，
        // 把绕圈打断。实测后如果真的有问题，可选方案：a) hasCustomOrbitCenter单独设一个
        // 离边界更远的绕圈中心点（参照海加尔山）b) 重新探路把西侧边界往外扩 c) 半径改小。
        t.deploymentStart = { 16403.500000f, 16297.299805f, 130.120071f, 0.0f };
